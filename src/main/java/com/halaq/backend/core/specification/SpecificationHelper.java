...
import com.halaq.backend.core.util.NumberUtil;
...
public abstract class SpecificationHelper<Criteria extends BaseCriteria, H extends BaseEntity> implements Specification<H> {
    ...
    public void addPredicate(String name, Boolean value,Operator operator) {
        addPredicate(name, value);
        addPredicate(name, value,operator);
    }
    ...
    public void addPredicate(String name, Boolean value) {
        if (value != null) {
    ...
    public void addPredicateBigDecimal(String name, String valueMin,String valueMax) {
        if (StringUtil.isNotEmpty(valueMin)) {
            ...
            predicates.add(builder.lessThanOrEqualTo(root.
                    <BigDecimal>get(name), new BigDecimal(valueMax)));
        }
    }
